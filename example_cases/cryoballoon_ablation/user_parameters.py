...
stabilization_parameters = dict(	
		...
		# Navier-stokes
		SUPG_NS = True,							# implicit
		PSPG_NS = False,						# implicit		
		crosswind_NS = True,					# implicit

		# Energy-equation
		SUPG_HT = True,							# implicit
		crosswind_HT = True						# implicit
	)
...
physical_parameters = dict(
		...
		nu = 3.18e-3,							# Dynamic viscosity (kg/m.s)
		Spht_f = 3820,         					# Specific heat (J/kg.C)
		K_f = 0.45,								# Thermal conductivity (W/m.C)
...
time_control = dict(
		...
		C_no = 0.5,								# Set convection CFL number (recommended - for time accuracy : C_no < 1.0, max possible is 5.0)
		C_vi = 5,								# Set viscous CFL number (max possible is 30)
		C_kn = 5,								# Set conduction CFL number (max possible is max(C_vi/Pr, thermal_diff_ratio*C_vi/Pr))
		dt = 0.005,  							# Time-step: constant throughout runtime if adjustable-timestep is "False"
		T = 1,									# Total runtime
		dt_min = 0.002,							# Minimum permissible time step in % of min cell size (recommended - 2%)
