...
def fluid_create_boundary_conditions(fluid_mesh, **V):
	...
	bcu_top_x = DirichletBC(V['fluid'][0], Constant(1), boundaries, 5)
	...
	bcu_top_y = DirichletBC(V['fluid'][0], Constant(0), boundaries, 5)
	...
	bcu_top_z = DirichletBC(V['fluid'][0], Constant(0), boundaries, 5)
	bcu_z = [bcu_left_z, bcu_bottom_z, bcu_top_z]
...
def solid_create_boundary_conditions(solid_mesh, boundaries, dt, **V):
	...
	if problem_physics['compressible_solid'] == False:
		bcx_flag = DirichletBC(V['solid'][1].sub(0), Constant((0, 0, 0)), boundaries, 1)
	elif problem_physics['compressible_solid'] == True:
	    bcx_flag = DirichletBC(V['solid'][0], Constant((0, 0, 0)), boundaries, 1)
